define(function(require, exports, module) {
	...
	class FinLife {
		...
		app() {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				bridge.registerHandler('goBackBtnHandler', function(data, responseCallback) {
					let backLink = "/view/2017/thirdAnnicersary/index.html?interaction=1&userId=" + getUrlParam('userId');
				...
				$(".bbs").on("click", function() {
					let id = $(this).data('id');
					let backLink = webUrl + "/view/2017/thirdAnnicersary/about/index.html?interaction=1&userId=" + getUrlParam('userId');
				...
				$(document).on('click', '._finLife ._btn', function() {
					let currentHref = webUrl + "/view/2017/thirdAnnicersary/about/index.html?interaction=1&userId=" + getUrlParam('userId');
		...
		loadEvents(year) {
			let evtsHtml = '';
			let evts = events[year];
			for (let key in evts) {
				evtsHtml += `<p><span class="month">${key.split("m")[1]}月:</span>`;
				for (let i = 0; i < evts[key].length; i++) {
